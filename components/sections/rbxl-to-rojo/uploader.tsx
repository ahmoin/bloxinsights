...
interface RbxlToRojoWasmModule {
  convert: (
    bytes: Uint8Array,
    extension: string,
    projectName: string,
...
let wasmModulePromise: Promise<RbxlToRojoWasmModule> | null = null;

function loadWasmModule(): Promise<RbxlToRojoWasmModule> {
  if (!wasmModulePromise) {
    wasmModulePromise = (async () => {
      const wasmModule = (await import(
        /* webpackIgnore: true */ WASM_MODULE_URL
      )) as RbxlToRojoWasmModule;
...
function zipFromFiles(files: ConvertedFile[]): Uint8Array {
  const encoder = new TextEncoder();
  const entries: Record<string, Uint8Array> = {};
  for (const file of files) {
    entries[file.path] = encoder.encode(file.contents);
  }
...
function FileExplorer({
  ...
  const [excludeMetaFiles, setExcludeMetaFiles] = useState(false);
  const visibleFiles = useMemo(
    () =>
      excludeMetaFiles
        ? result.files.filter((file) => !isMetaFile(file.path))
        : result.files,
    [excludeMetaFiles, result.files]
  );
  ...
  const handleDownload = () => {
    const zip = excludeMetaFiles ? zipFromFiles(visibleFiles) : result.zip;
    downloadZip(zip, result.projectName);
  };
      ...
      <div className="flex items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground text-sm">
            {visibleFiles.length} files ready. Browse the project before
        ...
        <div className="flex shrink-0 items-center gap-4">
          <div className="flex items-center gap-2">
            <Checkbox
              checked={excludeMetaFiles}
              id="exclude-meta-post"
              onCheckedChange={(checked) =>
                setExcludeMetaFiles(checked === true)
              }
            />
            <Label className="font-normal text-sm" htmlFor="exclude-meta-post">
...
export function RbxlToRojoUploader() {
  ...
  const processFile = async (file: File) => {
    ...
    if (!extension) {
      toast.error("File must be a .rbxl, .rbxlx, .rbxm, or .rbxmx file");
      return;
    }
    ...
    try {
      ...
      toast.error(`This file couldn't be converted: ${message}`);
    } finally {
    ...
    >
      <EmptyHeader className="max-w-lg gap-4">
        ...
        <EmptyDescription className="text-base">
          Drag and drop or upload a .rbxl, .rbxlx, .rbxm, or .rbxmx file.
