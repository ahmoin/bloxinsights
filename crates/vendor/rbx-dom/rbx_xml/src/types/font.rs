...
impl XmlType for Font {
    ...
    fn read_xml<R: Read>(reader: &mut XmlEventReader<R>) -> Result<Self, DecodeError> {
        ...
        // Patchwork fix for Roblox files with an empty `Weight` tag
        // (e.g. `<Weight></Weight>`), which have no numeric content to parse.
        let weight_text = reader.read_tag_contents("Weight")?;
